...
1. Apply the RDF Skeleton to the spreadsheet using Google Refine
	...
	2. Remove the remove first row after importing. More specifically, set "Parse next 1 line as column header" and 
	"Discard initial 1 row of data". The other values should be zero.
...
3. The python script does not create all the triples, it depends on the types of changes. For now, the script deals with Narrow/Broad
   relations between the codes, the status of the codes and the replacedBy/replaces relations for the codes that have a change of type
   (recoded/boundary shift/boundary change). If other changes may occur, extra triples need to be created.
   This can be done by extending the script or by creating the triples manually.
...
5. Upload the consolidated file to virtuoso, from which an RDF can be retrieved.
